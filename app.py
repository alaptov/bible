...
def get_strongs_concordance():
    """Get unified Strong's Concordance data combining Hebrew and Greek entries"""
    from models import StrongsHebrew, StrongsGreek
    ...
    hebrew_entries = hebrew_query.order_by(StrongsHebrew.strong_number).limit(50).all()
    greek_entries = greek_query.order_by(StrongsGreek.strong_number).limit(50).all()
    
    # Combine all entries into a single list
    all_entries = []
    
    # Add Hebrew entries with root meanings
    for entry in hebrew_entries:
        entry_dict = entry.to_dict()
        entry_dict['language'] = 'Hebrew'
        
        # Generate root meaning from pictographic analysis
        entry_dict['root_meaning'] = generate_root_meaning(entry.hebrew_word)
    ...
    for entry in greek_entries:
        entry_dict = entry.to_dict()
        entry_dict['language'] = 'Greek'
        entry_dict['root_meaning'] = None  # Greek entries don't have paleo analysis
        all_entries.append(entry_dict)
    
    # Sort all entries by Strong's number (H1, H2... then G1, G2...)
    all_entries.sort(key=lambda x: (x['strong_number'][0], int(x['strong_number'][1:])))
...
def generate_root_meaning(hebrew_word):
    """Generate pictographic root meaning analysis for Hebrew words"""
    if not hebrew_word:
        return None
        
    from models import PaleoLetter
    from utils.hebrew_converter import remove_nikud, hebrew_to_paleo
    
    try:
        # Clean the Hebrew word (remove nikud/vowel points)
        ...
        # Analyze each letter pictographically
        letter_meanings = []
        pictographic_parts = []
        
        for char in clean_hebrew:
            if char and char != ' ':
                letter = PaleoLetter.query.filter_by(letter=char).first()
                if letter:
                    # Get the first/primary meaning
                    primary_meaning = letter.meaning.split(',')[0].strip()
                    letter_meanings.append(primary_meaning)
                    pictographic_parts.append(f"{letter.name} ({letter.paleo_symbol}) = {primary_meaning}")
        
        if not letter_meanings:
            return None
            
        # Create the root meaning structure
        root_meaning = {
            'paleo_word': paleo_word,
            'pictographic_analysis': ' + '.join(letter_meanings),
            'formation_explanation': '; '.join(pictographic_parts),
            'original_concept': f"Ancient pictographic meaning: {' + '.join(letter_meanings)}"
        }
        
        return root_meaning
        
    except Exception as e:
        print(f"Error generating root meaning for {hebrew_word}: {e}")
        return None
