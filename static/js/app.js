    ...
    try {
        ...
        booksContainer.innerHTML = books.map(book => `
            <div class="book-card" onclick="showBookChapters(${book.id})">
                ...
                ${book.testament !== 'New Testament' ? `<div class="book-phonetic">${book.phonetic_name}</div>` : ''}
...
async function showBookChapters(bookId) {
    try {
        ...
        document.getElementById('current-book-title').textContent = `${book.name} (${book.phonetic_name})`;
    ...
    try {
        ...
        versesContainer.innerHTML = chapter.verses.map(verse => `
            <div class="verse-card">
                ...
                <div class="verse-paleo">${makePaleoWordsInteractive(verse.paleo_text, verse.paleo_transliteration, verse)}</div>
                <div class="verse-paleo-transliteration"><strong>${verse.is_new_testament ? 'Greek:' : 'Paleo:'}</strong> ${verse.paleo_transliteration}</div>
        ...
        ${results.results.map(verse => `
            <div class="search-result">
                <div class="result-header">
                    ...
                    <button class="tts-btn" onclick="speakVerse('${verse.id}', '${verse.paleo_transliteration.replace(/'/g, "\\'")}', '${verse.paleo_text.replace(/'/g, "\\'")}')">
                        <i class="fas fa-play"></i>
                    </button>
                </div>
                <div class="verse-paleo">${makePaleoWordsInteractive(verse.paleo_text, verse.paleo_transliteration, verse)}</div>
...
async function speakVerse(verseId, paleoTransliteration, paleoText) {
    // Legacy function for backward compatibility
    return speakVerseEnhanced(verseId, {
        id: verseId,
        paleo_text: paleoText,
        paleo_transliteration: paleoTransliteration
    });
}
        ...
        html += `
            <div class="strongs-entry">
                <div class="strongs-header">
                    <span class="strong-number">${entry.strong_number}</span>
                    <span class="language-badge ${entry.language.toLowerCase()}">${entry.language}</span>
                    <div class="word-display">
                        ${entry.language === 'Hebrew' && entry.hebrew_word ? 
                            `<div class="paleo-word">${entry.word}</div>` : 
                            `<div class="${entry.language.toLowerCase()}-word">${entry.word}</div>`
                        }
...
function makePaleoWordsInteractive(paleoText, paleoTransliteration, verseData = null) {
    if (!paleoText) return paleoText || '';
    ...
    // Clean transliteration text of HTML tags for word matching
    const cleanTransliteration = paleoTransliteration ? paleoTransliteration.replace(/<[^>]*>/g, ' ') : '';
    
    // Split both texts into words for matching
    const paleoWords = tempPaleoText.split(/(\s+)/);
    const transliterationWords = cleanTransliteration.split(/(\s+)/);
    ...
    let result = paleoWords.map((part, index) => {
        ...
        if (isPaleoWord) {
            // This is a Paleo Hebrew word - get corresponding transliteration
            const correspondingTranslit = transliterationWords[index] ? transliterationWords[index].trim() : '';
            ...
            return `<span class="paleo-word-interactive" onclick="showWordOverlay('${trimmedPart.replace(/'/g, "\\'")}', '${correspondingTranslit.replace(/'/g, "\\'")}', ${wordPosition}, '${verseId}')" title="Click for root analysis">${part}</span>`;
        } else {
...
async function showWordOverlay(paleoWord, transliteration, wordPosition = -1, verseId = 'unknown') {
    const overlay = document.getElementById('word-overlay');
    const loadingMessage = 'Loading word analysis...';
    
    try {
        // Show overlay with loading state - use provided Paleo word and transliteration
        document.getElementById('overlay-paleo-word').textContent = paleoWord;
        document.getElementById('overlay-transliteration').textContent = transliteration;
        ...
        // Direct paleo script analysis - no Hebrew conversion needed
        let wordData = null;
        
        console.log('Analyzing Paleo word:', paleoWord, 'with transliteration:', transliteration);
        
        // Generate direct analysis from paleo letters
        wordData = generateDirectPaleoAnalysis(paleoWord, transliteration);
        ...
        if (wordData) {
            ...
            showWordNotFound(paleoWord);
        }
        
    } catch (error) {
        console.error('Error fetching word analysis:', error);
        showWordError(paleoWord);
    }
}

function populateWordOverlay(wordData) {
    // Display Paleo → Transliteration → English sequence
    document.getElementById('overlay-paleo-word').textContent = wordData.paleo_word;
    document.getElementById('overlay-transliteration').textContent = wordData.transliteration;
    
    // Get English word from COMPLETE Paleo mapping or analysis
    const paleoWord = wordData.paleo_word;
    let englishWord = wordData.english_meaning || 'word'; // default fallback
...
function generateDirectPaleoAnalysis(paleoScript, transliteration) {
...
function showWordNotFound(paleoWord) {
    document.getElementById('overlay-paleo-word').textContent = paleoWord;
    document.getElementById('overlay-transliteration').textContent = '';
    document.getElementById('overlay-english-word').textContent = 'unknown';
    document.getElementById('overlay-pictographic-breakdown').textContent = `No analysis available for "${paleoWord}". This word may be a variant or not in our dictionary.`;
...
function showWordError(paleoWord) {
    document.getElementById('overlay-paleo-word').textContent = paleoWord;
    document.getElementById('overlay-transliteration').textContent = '';
